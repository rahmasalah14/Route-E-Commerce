...
async function subCategoryDetails({params}:{params:Params}) {
 
       let {subId}=await params
        
             const res= await fetch('https://ecommerce.routemisr.com/api/v1/subcategories/'+subId)
             const {data:subCategory}:{data:Subcategory} = await res.json()
  ...
  if(subCategory)
    ...
    (product) => product.subcategory[0]?._id === subCategory._id);
